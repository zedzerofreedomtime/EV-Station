...
const schema = z.object({
  ...
  googleMapsUrl: z.union([z.literal(''), z.url('Enter a valid URL')]).optional(),
  landSize: z.preprocess(value => Number(value), z.number().positive('Land size must be greater than zero')),
  ...
  if (!hasAddress && !(hasLat && hasLng)) ctx.addIssue({ code: 'custom', path: ['address'], message: 'Provide an address or latitude and longitude.' })
  if (hasLat !== hasLng) ctx.addIssue({ code: 'custom', path: [hasLat ? 'longitude' : 'latitude'], message: 'Both coordinates are required.' })
  if (hasLat && (value.latitude! < -90 || value.latitude! > 90)) ctx.addIssue({ code: 'custom', path: ['latitude'], message: 'Latitude must be between -90 and 90.' })
  if (hasLng && (value.longitude! < -180 || value.longitude! > 180)) ctx.addIssue({ code: 'custom', path: ['longitude'], message: 'Longitude must be between -180 and 180.' })
})
...
export function NewSitePage() {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: { landSizeUnit: 'sqm', address: '', googleMapsUrl: '', notes: '' } })
  const mutation = useMutation({ mutationFn: api.createSite, onSuccess: async site => { await queryClient.invalidateQueries({queryKey:['sites']}); navigate(`/sites/${site.id}`) } })
  ...
  return <div><div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="page-title">{t('New Site')}</h1><p className="mt-2 text-sm text-muted">{t('Record customer-supplied site details before verification.')}</p></div><div className="inline-flex items-center gap-2 text-sm text-muted"><Info size={16}/>{t('Saved inputs are not yet verified.')}</div></div>
    <form onSubmit={handleSubmit(values => mutation.mutate(schema.parse(values)))} className="mt-8 grid gap-6 pb-24 xl:grid-cols-[minmax(0,0.95fr)_minmax(460px,1.05fr)]">
      <section className="rounded-xl border border-line bg-white p-6 shadow-panel"><h2 className="section-title">{t('Location')}</h2><div className="mt-5 space-y-5">
        ...
        {geocoding.isError ? <p className="text-sm text-red-600">{geocoding.error.message}</p> : null}
        ...
        <label className="field"><span>{t('Google Maps URL')}</span><input type="url" {...register('googleMapsUrl', { onBlur: event => { const coordinates = extractGoogleMapsCoordinates(event.target.value); if (coordinates) { setValue('latitude', coordinates.latitude, { shouldValidate: true }); setValue('longitude', coordinates.longitude, { shouldValidate: true }) } } })} placeholder="https://maps.google.com/…"/><p className="field-hint">{t('Paste a Google Maps link and the coordinates will be filled automatically.')}</p><FieldError message={errors.googleMapsUrl?.message}/></label>
      ...
      <div className="flex items-center justify-end gap-3 xl:fixed xl:bottom-0 xl:left-[236px] xl:right-0 xl:z-20 xl:border-t xl:border-line xl:bg-white xl:px-10 xl:py-4"><button type="button" className="button-secondary" onClick={() => navigate('/')}>{t('Cancel')}</button><button type="submit" className="button-primary" disabled={mutation.isPending}>{mutation.isPending ? t('Saving…') : t('Save Site')}</button></div>
      {mutation.isError && <p className="text-right text-sm text-red-600 xl:col-span-2">{mutation.error.message}</p>}
    </form>
  </div>
}
